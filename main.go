...
func main() {
    ...
    if err := os.MkdirAll(metricsDir, 0755); err != nil {
        log.Error().Err(err).Str("directory", metricsDir).Msg("Failed to create metrics directory")
        // Fallback to current directory if creation fails, or handle error as preferred
    }

    // Export metrics to a file within the metrics directory
    metricsFilePath := fmt.Sprintf("%s/metrics_%s_%s.json", metricsDir, language, time.Now().Format("2006-01-02"))
...
func processCSV(filePath string, privateKey string, today string, metrics *MetricsCollector) {
    ...
    relays := []string{
        ...
        "wss://haven.bitcoin-calendar.org",
    }
    ...
    for i, record := range records[1:] { // Skip header
        ...
        if eventMonthDay == today {
            ...
            tags := nostr.Tags{
                nostr.Tag{"calendar", "history", "bitcoin", "bitcoincalendar", "onthisday"},
